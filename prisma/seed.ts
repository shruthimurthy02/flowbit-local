...
async function main() {
  // Find data file - use absolute path
  const absolutePath = "C:\\Users\\shrut\\OneDrive\\Desktop\\flowbit-intern-assignment\\data\\Analytics_Test_Data.json";
  const cwd = process.cwd();
  
  const possiblePaths = [
    absolutePath,
    path.join(cwd, "data", "Analytics_Test_Data.json"),
    path.join(cwd, "..", "..", "data", "Analytics_Test_Data.json"),
    path.resolve(__dirname || cwd, "..", "data", "Analytics_Test_Data.json"),
  ];
  ...
  if (data.vendors && Array.isArray(data.vendors)) {
    for (const vendor of data.vendors) {
      ...
      // Use findFirst + create/update since name is unique
